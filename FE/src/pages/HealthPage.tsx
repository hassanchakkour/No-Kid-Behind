...
export default function HealthPage() {
  ...
  return (
    <Box sx={{ bgcolor: 'background.default', minHeight: '100vh' }}>
      ...
      <Box
        sx={{
          background: 'linear-gradient(135deg, #2e5d8e 0%, #1a3d61 100%)',
          px: { xs: 4, md: 8 },
          pt: 5,
          pb: 5,
          position: 'relative',
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 4 }}>
            <Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                <SpaRoundedIcon sx={{ color: 'rgba(166,210,255,0.7)', fontSize: '0.875rem' }} />
                <Typography sx={{ fontWeight: 700, fontSize: '0.6875rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(166,210,255,0.7)' }}>
                  Mental Health & Wellbeing
                </Typography>
              </Box>
              <Typography
                component="h1"
                sx={{ fontWeight: 800, fontSize: { xs: '2.25rem', md: '3rem' }, letterSpacing: '-0.035em', color: '#e8f4ff', lineHeight: 1.05, mb: 1.5 }}
              >
                Health Resources
              </Typography>
            ...
            <Box
              sx={{
                ...
                border: '1px solid rgba(166,210,255,0.2)',
                borderRadius: '12px',
                px: 4,
                py: 3,
                textAlign: 'center',
            ...
            >
              <Typography sx={{ fontWeight: 800, fontSize: '2.5rem', letterSpacing: '-0.05em', color: '#e8f4ff', lineHeight: 1 }}>
                {isLoading ? '…' : allVideos?.length ?? 0}
              </Typography>
              <Typography sx={{ fontWeight: 700, fontSize: '0.625rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(166,210,255,0.6)', mt: 0.5 }}>
                Videos Available
              </Typography>
            </Box>
          </Box>
      ...
      <Box sx={{ bgcolor: 'background.paper', borderBottom: '1px solid rgba(169,180,185,0.1)', px: { xs: 4, md: 8 }, py: 3, position: 'sticky', top: 64, zIndex: 50 }}>
        <Box sx={{ maxWidth: 1280, mx: 'auto', display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'text.secondary', flexShrink: 0 }}>
            <FavoriteBorderRoundedIcon sx={{ fontSize: '1rem' }} />
            <Typography sx={{ fontWeight: 600, fontSize: '0.8125rem' }}>Filter</Typography>
            {activeFilters > 0 && (
              <Chip
                label={activeFilters}
                size="small"
                sx={{ bgcolor: '#2e5d8e', color: '#e8f4ff', fontWeight: 700, fontSize: '0.625rem', height: 18, ml: 0.5 }}
              />
            )}
          </Box>
      ...
      <Box sx={{ px: { xs: 4, md: 8 }, py: 8, maxWidth: 1280, mx: 'auto' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 5 }}>
            ...
            {activeFilters > 0 && (
              <Typography
                ...
                sx={{ fontSize: '0.8125rem', color: '#2e5d8e', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}
              >
                Clear filters
              </Typography>
            )}
        ...
        {isLoading ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', py: 14 }}>
            <CircularProgress sx={{ color: '#2e5d8e' }} />
          </Box>
        ) : filtered.length === 0 ? (
          <Box
            sx={{
          ...
          >
            <SpaRoundedIcon sx={{ fontSize: '3rem', color: 'text.disabled', mb: 2 }} />
